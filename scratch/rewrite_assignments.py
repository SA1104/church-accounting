...
// Helper to generate assignment code with retry
async function generateAndInsertAssignment(insertFn, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      const seq = await query.get("SELECT nextval('public.church_assignment_code_seq') as seq");
      const code = `A-${String(seq.seq).padStart(6, '0')}`;
...
router.get('/me', authenticateToken, async (req, res) => {
  try {
    ...
    const assignments = await query.all(`
      SELECT 
        a.id, a.committee_id, a.group_id, a.position_id, a.role_id, a.role_code, a.assignment_code,
        a.is_primary, a.is_active, a.assigned_at, a.revoked_at, a.status,
      ...
      ORDER BY a.is_active DESC, a.is_primary DESC, a.assigned_at ASC
    `, [userId, projectId]);
...
router.post('/apply', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const result = await generateAndInsertAssignment(async (code) => {
      return await query.run(`
        INSERT INTO public.church_user_assignments 
          (user_id, project_id, committee_id, group_id, position_id, role_id, role_code, assignment_code, is_primary, is_active, status, assigned_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, FALSE, TRUE, 'pending', CURRENT_TIMESTAMP)
        RETURNING id
      `, [userId, projectId, committee_id, group_id || null, position_id, pos.role_code, pos.role_code, code]);
    });
...
router.get('/admin/assignments/pending', authenticateToken, async (req, res) => {
  try {
    ...
    const pending = await query.all(`
      SELECT 
        a.id, a.assigned_at, a.status, a.assignment_code,
...
router.post('/users/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  const { committee_id, group_id, position_id, role_id, role_code, is_primary } = req.body;
  if (!position_id && !role_code && !role_id) return res.status(400).json({ message: '직책 또는 역할 코드가 필요합니다.' });
  try {
    const projectId = await getActiveProjectId(req);
    if (!req.user.isAdmin) {
      return res.status(403).json({ message: '교회 관리자 권한이 필요합니다.' });
    ...
    // Resolve role_code and role_id
    let resolvedRoleCode = role_code;
    let resolvedRoleId = role_id || role_code;
    if (position_id && !resolvedRoleCode) {
      ...
      if (pos) {
        resolvedRoleCode = pos.role_code;
        if (!resolvedRoleId) resolvedRoleId = pos.role_code;
      }
    }
    ...
    const result = await generateAndInsertAssignment(async (code) => {
      return await query.run(`
        ...
        RETURNING id
      `, [id, projectId, committee_id || null, group_id || null, position_id || null, resolvedRoleId, resolvedRoleCode, code, is_primary ? true : false]);
    });
...
// Note: History is not deleted. We just revoke it.
router.delete('/users/:id/:assignmentId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await query.run(
      'UPDATE public.church_user_assignments SET is_active = FALSE, updated_by = ?, revoked_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ? AND project_id = ?',
      [req.user.userId || req.user.id, assignmentId, id, projectId]
    );
...
// PUT /api/church/assignments/users/:id/:assignmentId -- update assignment
router.put('/users/:id/:assignmentId', authenticateToken, async (req, res) => {
  const { id, assignmentId } = req.params;
  const { committee_id, group_id, position_id, role_id, role_code, is_primary } = req.body;
  try {
    const projectId = await getActiveProjectId(req);
    ...
    if (!assignment) return res.status(404).json({ message: '배정 정보를 찾을 수 없습니다.' });

    let resolvedRoleCode = role_code || assignment.role_code;
    let resolvedRoleId = role_id || assignment.role_id;
    if (position_id && position_id !== assignment.position_id) {
      const pos = await query.get('SELECT role_code FROM public.church_positions WHERE position_id = ?', [position_id]);
      if (pos) {
        resolvedRoleCode = pos.role_code;
        resolvedRoleId = pos.role_code;
      }
    }
    ...
    await query.run(
      'UPDATE public.church_user_assignments SET committee_id = ?, group_id = ?, position_id = ?, role_id = ?, role_code = ?, is_primary = ?, updated_by = ? WHERE id = ?',
      [committee_id, group_id || null, position_id, resolvedRoleId, resolvedRoleCode, is_primary, req.user.userId || req.user.id, assignmentId]
    );

    const newAssign = { committee_id, group_id, position_id, role_code: resolvedRoleCode };
    await logAssignmentHistory(projectId, id, assignmentId, assignment, newAssign, 'updated', req.user.userId || req.user.id, '관리자에 의한 배정 수정');

    res.json({ success: true, message: '배정이 수정되었습니다.' });
  } catch (err) {
    console.error('[ASSIGNMENTS] Error updating assignment:', err);
    res.status(500).json({ success: false, message: '배정 수정 중 오류가 발생했습니다.', details: err.message });
  }
});
