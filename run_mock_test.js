...
async function runMockTest() {
  ...
  // 1. KRX ?ìž¥??ë¬¸ìž?´ì´ ? íš¨??ISO ? ì§œë¡?ë³€?˜ë¨ + ?¼ì„±?„ìž/?°ì„ ì£?+ ?˜ëª»??? ì§œ ê²€ì¦?  const rawData = [
  ...
  const inst = insts.rows.find(r => r.stock_code === '005930'); console.log('listing_date DB value:', inst.listing_date); if (!inst.listing_date || inst.listing_date.toISOString().startsWith('1975-06-11') === false) throw new Error('listing_date not persisted');
  if (venues.rows.length !== 2) throw new Error('Venues not created exactly 1 per instrument');

  console.log('Mock Repository Upsert OK');

  // 3. Update Existing and Idempotent Venues
  s5930.listing_date = '1975-06-12'; // Modify date
  ...
  if (insts2.rows[0].listing_date.toISOString().startsWith('1975-06-12') === false) throw new Error('listing_date not updated');
