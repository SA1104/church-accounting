...
async function run() {
  ...
  console.log('--- 1. Backfill Final Verification ---');
  
  // stock_daily_bars 총 행 수
  const barsCount = await client.query(`SELECT COUNT(*) FROM stock_daily_bars`);
  console.log('stock_daily_bars 총 행 수:', barsCount.rows[0].count);
  
  // distinct trade_date
  const distDates = await client.query(`SELECT COUNT(DISTINCT trade_date) FROM stock_daily_bars`);
  console.log('distinct trade_date:', distDates.rows[0].count);
  
  // MIN/MAX trade_date
  const minMaxDates = await client.query(`SELECT MIN(trade_date), MAX(trade_date) FROM stock_daily_bars`);
  console.log('MIN/MAX trade_date:', minMaxDates.rows[0]);
  
  // 연도별 행 수
  const yearlyCounts = await client.query(`
    SELECT EXTRACT(YEAR FROM trade_date) as yr, COUNT(*) 
    FROM stock_daily_bars 
    GROUP BY yr ORDER BY yr
  `);
  console.log('연도별 행 수:');
  yearlyCounts.rows.forEach(r => console.log(`  ${r.yr}: ${r.count}`));
  
  // 중복 PK 0건 (instrument_id, trade_date)
  ...
  `);
  console.log('중복 PK 0건 검증:', dupes.rowCount === 0 ? '통과' : '실패');
  ...
  // 음수 거래량·거래대금 0건
  const negVol = await client.query(`SELECT COUNT(*) FROM stock_daily_bars WHERE trading_volume < 0 OR trading_value < 0`);
