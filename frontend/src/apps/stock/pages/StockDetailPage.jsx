...
export default function StockDetailPage() {
  ...
  if (status === 'loading') {
    return <div className="flex justify-center py-20"><span className="text-slate-400">Loading...</span></div>;
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 max-w-5xl mx-auto space-y-4">
        ...
        <div className="text-center">
          <p className="text-lg font-bold text-white mb-1">시장 데이터 연결 준비 중</p>
          <p className="text-sm text-slate-400">{error || '데이터베이스 갱신이 필요합니다.'}</p>
        </div>
        <div className="flex gap-2 mt-4">
          <Link to="/stock" className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-semibold transition-colors">
            이전으로
          </Link>
          <button onClick={refetch} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold transition-colors">
            다시 시도
          </button>
        </div>
      </div>
  ...
  if (status === 'error') {
    return (
      <div className="flex flex-col items-center justify-center py-20 max-w-5xl mx-auto space-y-4">
        ...
        <div className="text-center">
          <p className="text-lg font-bold text-white mb-1">데이터 오류</p>
          <p className="text-sm text-slate-400">{error || '알 수 없는 오류가 발생했습니다.'}</p>
        </div>
        <button onClick={refetch} className="px-4 py-2 mt-4 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-semibold transition-colors">
          다시 시도
        </button>
      </div>
    );
  ...
  if (status === 'success' && !instrument) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <div className="text-slate-500 mb-2">찾을 수 없는 종목코드입니다: {stockCode}</div>
        <p className="text-sm text-slate-400">올바른 종목코드인지 확인해주세요.</p>
        <Link to="/stock" className="mt-4 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-semibold transition-colors">
          목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="max-w-5xl mx-auto space-y-6">
        <Link to="/stock/stocks" className="inline-flex items-center text-xs font-semibold text-slate-400 hover:text-white transition-colors mb-2">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-2xl font-bold text-white">{instrument.instrument_name}</h1>
              <span className="text-sm text-slate-400 font-semibold">{instrument.stock_code}</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-semibold transition-colors">
              <Share2 size={14} /> 공유
            </button>
            <button className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold transition-colors shadow shadow-blue-900/50">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-end">
            <h2 className="text-base font-bold text-white">가격 흐름 <span className="text-slate-500 text-xs font-normal ml-2">{bars.length}일 데이터</span></h2>
            <StockComparisonSelector selected={comparison} onChange={setComparison} />
          </div>
          {bars.length > 0 ? (
            <StockChartPlaceholder height="300px" />
          ) : (
            <div className="h-[300px] bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center text-slate-500 text-sm">
              일별 데이터가 없습니다.
            </div>
          )}
        </div>
        ...
        <div>
          <h2 className="text-base font-bold text-white mb-3">핵심 지표</h2>
        ...
        <div>
          <h2 className="text-base font-bold text-white mb-3">공시 및 뉴스</h2>
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-base font-bold text-white">관련 분석글</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {mockCommunityPosts.filter(p => p.stockCode === stockCode).length > 0 ? (
              ...
              <div className="col-span-1 md:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-6 text-center text-slate-500 text-sm">
                관련 분석글이 없습니다.
              </div>
            )}
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/50 text-[10px] text-slate-500 space-y-1">
